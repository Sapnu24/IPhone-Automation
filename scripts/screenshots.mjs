...
const SEED = `(${async () => {
  ...
  await put('ledgers', { id: 'led-1', direction: 'debt', person: 'Pag-IBIG Housing Loan', amount: 10500, settledAmount: 0, note: 'Monthly amortization', createdAt: now })
  await put('ledgers', { id: 'led-2', direction: 'debt', person: 'Kuya', amount: 2000, settledAmount: 500, createdAt: now })
  await put('ledgers', { id: 'led-3', direction: 'owed', person: 'Carlo', amount: 260, settledAmount: 0, note: 'Grab split', dueDate: d(-5), createdAt: now })
  await put('ledgers', { id: 'led-4', direction: 'owed', person: 'Maya', amount: 6000, settledAmount: 0, createdAt: now })
  await put('notes', { id: 'note-1', text: 'Ipon challenge: save ₱500/week for the Japan trip 🇯🇵', pinned: true, createdAt: now, updatedAt: now })
  await put('notes', { id: 'note-2', text: 'Cancel unused subscriptions before payday.', createdAt: now, updatedAt: now - 1000 })
}})()`
...
await page.screenshot({ path: `${outDir}/08-insights-full.png`, fullPage: true })
await page.getByRole('link', { name: 'More' }).click()
await page.getByRole('button', { name: /Debt/ }).click()
await shot('08b-debt')
await page.getByRole('link', { name: 'More' }).click()
await page.getByRole('button', { name: /Notes/ }).click()
await shot('08c-notes')
await page.getByRole('link', { name: 'More' }).click()
await page.getByRole('button', { name: /Rewards/ }).click()
await shot('08d-rewards')
await page.getByRole('link', { name: 'More' }).click()
await page.getByRole('button', { name: /Tools/ }).click()
await shot('08e-tools')
await page.getByRole('button', { name: /Currency exchange/ }).click()
await shot('08f-converter')
await page.getByRole('link', { name: 'More' }).click()
await page.getByRole('button', { name: /Coach/ }).click()
await shot('08g-coach')
